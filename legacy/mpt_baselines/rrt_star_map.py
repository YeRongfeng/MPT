...
from legacy.mpt_baselines._bootstrap import enable_local_packages
...
from legacy.mpt_baselines.generateMaps import generate_random_maps
from legacy.mpt_baselines.generateMazeMaps import generate_random_maze
...
def get_path(start, goal, ValidityCheckerObj=None):
    ...
    '''
    # 使用ValidityCheckerObj中的SpaceInformation
    si = ValidityCheckerObj.getSpaceInformation()
...
def start_experiment_rrt(start, samples, fileDir=None):
    ...
    # Validity checking
    ValidityCheckerObj = ValidityChecker(si, CurMap=CurMap, res=dist_resl, robot_radius=robot_radius)
...
if __name__ == "__main__":
    ...
    def process_environment(env_index, env_type, base_file_dir, 
        ...
        if env_type == 'realworld':
            ...
            if env_type == 'forest':
                generate_random_maps(
                    width=map_width * 0.05, 
                    height=map_height * 0.05, 
                    seed=1000 + env_index, 
                    fileName=env_map_file,
                    num_circle=69, 
                    num_box=29
                )
            elif env_type == 'maze':
