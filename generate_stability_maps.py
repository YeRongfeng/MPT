...
def cache_is_compatible(
    ...
    try:
        with np.load(output_file) as cached:
            return (
                cached["yaw_stability"].shape == (*map_shape, yaw_bins)
                and cached["cost_map"].shape == (*map_shape, yaw_bins)
                and int(cached["yaw_bins"]) == yaw_bins
                and np.isclose(
                    float(cached["voxel_size_xy"]),
                    resolution,
                    atol=1e-6,
                    rtol=0.0,
                )
                and np.isclose(
                    float(cached["yaw_weight"]),
                    yaw_weight,
                    atol=1e-6,
                    rtol=0.0,
                )
