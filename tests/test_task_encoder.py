"""Wiring tests for feature-level task_encoder (no training)."""
...
class TaskEncoderTests(unittest.TestCase):
    def test_factory_and_condition_routing(self):
        device = torch.device("cpu")
        baseline = SpatialMapPathMeanFlowTransformer(**PRODUCTION_ARGS)
        model, _ = build_model("task_encoder", device)
        self.assertEqual(model.ARCHITECTURE_NAME, "task_encoder")
        self.assertEqual(
            condition_semantics("task_encoder"),
            "task_encoder_feature_fuse_v2",
        )
        self.assertIn("task_encoder", ARCHITECTURES)
        self.assertFalse(hasattr(model, "cond_mlp"))
        self.assertFalse(hasattr(model, "pose_embedder"))
        self.assertFalse(hasattr(model, "rasterize_task_channels"))
        self.assertTrue(hasattr(model, "sg_mlp"))
        self.assertTrue(hasattr(model, "task_fuse"))
        self.assertTrue(hasattr(model, "time_cond_mlp"))
        self.assertEqual(model.map_fe_block1[0].in_channels, 4)
        self.assertEqual(
    ...
    def test_time_does_not_enter_task_memory(self):
        ...
        model = TaskEncoderSpatialPathMeanFlowTransformer(**SMALL_ARGS).to(device)
        ...
        self.assertTrue(torch.equal(feat_a["map_tokens"], feat_b["map_tokens"]))
        self.assertTrue(torch.equal(feat_a["task_vector"], feat_b["task_vector"]))
        self.assertFalse(torch.equal(feat_a["condition"], feat_b["condition"]))

    def test_start_goal_broadcast_and_map_change_task_memory(self):
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        model = TaskEncoderSpatialPathMeanFlowTransformer(**SMALL_ARGS).to(device)
        model.use_gradient_checkpoint = False
        model.eval()
        ...
        self.assertEqual(tuple(feat_a["map_tokens"].shape[1:]), (144, 128))
        self.assertEqual(tuple(feat_a["task_vector"].shape), (1, 128))
        self.assertTrue(torch.equal(feat_a["map_features"], feat_sg["map_features"]))
        self.assertFalse(torch.equal(feat_a["task_vector"], feat_sg["task_vector"]))
        self.assertFalse(torch.equal(feat_a["map_tokens"], feat_sg["map_tokens"]))
        self.assertFalse(torch.equal(feat_a["map_features"], feat_map["map_features"]))
        self.assertTrue(torch.equal(feat_a["task_vector"], feat_map["task_vector"]))
    ...
    def test_forward_backward_jvp_and_decoder(self):
        ...
        self.assertGreater(float(model.sg_mlp[0].weight.grad.abs().sum()), 0.0)
        self.assertGreater(float(model.task_fuse.weight.grad.abs().sum()), 0.0)
        self.assertGreater(
            float(model.map_fe_block1[0].weight.grad.abs().sum()), 0.0
        )
