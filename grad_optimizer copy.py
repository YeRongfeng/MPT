    ...
    def cost_function(self):
        ...
        if self.nx is not None and self.ny is not None and self.nz is not None:
            ...
            esdf_results = compute_esdf_batch(self.nx, self.ny, self.nz, queries,
                                              ...
                                              yaw_weight=0.7, search_radius=5.0, chunk_cells=3000, device=self.device)
            # 提取并转成tensor（原逻辑）
...
if __name__ == "__main__":
    ...
    # 执行优化
    optimized_trajectory, optimized_yaw_dense, cost_history = optimizer.optimize(iterations=300, lr=0.05)
