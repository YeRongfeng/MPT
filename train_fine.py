...
def generate_initial_trajectory_control_points(start_pose, goal_pose, num_middle_points=24, device='cpu', map_bounds=(-20, 20)):
    """
    生成初始轨迹的B样条控制点
    
    要求：
    - 一共26个控制点：第一个和最后一个固定为起点和终点
    - 第二个控制点：与第一个的连线需满足和起点朝向yaw一致
    - 倒数第二个控制点：与最后一个的连线需满足和终点朝向yaw一致
    - 其余中间点：直接线性插值
    - 第二个和倒数第二个点必须在地图范围内
    
    Args:
        start_pose: (B, 3) 或 (3,) 起点姿态 [x, y, yaw]
        goal_pose: (B, 3) 或 (3,) 终点姿态 [x, y, yaw]
        num_middle_points: 中间控制点数量（默认24）
        device: 设备
        map_bounds: tuple (min, max) 地图坐标范围，默认(-20, 20)
    
    Returns:
        full_control_points: (B, 26, 2) 或 (26, 2) 完整控制点
        middle_control_points: (B, 24, 2) 或 (24, 2) 中间控制点
    """
    ...
    distance = torch.norm(goal_xy - start_xy, dim=1, keepdim=True)
    second_point_distance = distance * 0.1
    start_direction = torch.stack([torch.cos(start_yaw), torch.sin(start_yaw)], dim=1)
    
    # 计算从起点出发沿着朝向方向能到达的最大距离（到达地图边界）
    max_dist_start = torch.full((B, 1), float('inf'), device=device)
    for i in range(B):
        dir_x, dir_y = start_direction[i, 0], start_direction[i, 1]
        pos_x, pos_y = start_xy[i, 0], start_xy[i, 1]
        
        # 计算沿方向能走到的最大距离（不超过地图范围）
        # 对于每个维度：dist = (bound - pos) / dir（当dir > 0时）
        dists = []
        if abs(dir_x) > 1e-6:
            dist_x_max = (map_max - pos_x) / abs(dir_x)
            dist_x_min = (map_min - pos_x) / abs(dir_x)
            dists.append(max(dist_x_max, dist_x_min))
        if abs(dir_y) > 1e-6:
            dist_y_max = (map_max - pos_y) / abs(dir_y)
            dist_y_min = (map_min - pos_y) / abs(dir_y)
            dists.append(max(dist_y_max, dist_y_min))
        
        if dists:
            max_dist_start[i, 0] = min(dists)
        else:
            max_dist_start[i, 0] = distance[i, 0] * 0.1
    
    # 约束第二个点的距离：不能超过最大距离
    second_point_distance = torch.min(second_point_distance, max_dist_start)
    second_cp = start_xy + start_direction * second_point_distance

    penultimate_point_distance = distance * 0.1
    goal_direction = torch.stack([torch.cos(goal_yaw), torch.sin(goal_yaw)], dim=1)
    
    # 计算从终点出发沿反朝向方向能到达的最大距离（到达地图边界）
    max_dist_goal = torch.full((B, 1), float('inf'), device=device)
    for i in range(B):
        dir_x, dir_y = -goal_direction[i, 0], -goal_direction[i, 1]  # 反向
        pos_x, pos_y = goal_xy[i, 0], goal_xy[i, 1]
        ...
        if dists:
            max_dist_goal[i, 0] = min(dists)
        else:
            max_dist_goal[i, 0] = distance[i, 0] * 0.1
    
    # 约束倒数第二个点的距离：不能超过最大距离
    penultimate_point_distance = torch.min(penultimate_point_distance, max_dist_goal)
    penultimate_cp = goal_xy - goal_direction * penultimate_point_distance
    
    # 再次约束所有控制点确保在地图范围内
    second_cp = torch.clamp(second_cp, map_min, map_max)
    penultimate_cp = torch.clamp(penultimate_cp, map_min, map_max)

    num_interpolation_points = num_middle_points - 2
    if num_interpolation_points > 0:
        t = torch.linspace(0, 1, num_interpolation_points + 2, device=device)[1:-1]
        t = t.unsqueeze(0).expand(B, -1)
        middle_points = second_cp.unsqueeze(1) + t.unsqueeze(2) * (penultimate_cp - second_cp).unsqueeze(1)
    else:
        middle_points = torch.empty(B, 0, 2, device=device)

    full_control_points = torch.cat([
        start_xy.unsqueeze(1),
        second_cp.unsqueeze(1),
        middle_points,
        penultimate_cp.unsqueeze(1),
        goal_xy.unsqueeze(1)
    ], dim=1)

    middle_control_points = full_control_points[:, 1:-1, :]

    if squeeze_back:
        full_control_points = full_control_points.squeeze(0)
        middle_control_points = middle_control_points.squeeze(0)

    return full_control_points, middle_control_points
...
def diffusion_loss(model, batch, device, loss_weights=None, epoch=0, total_epochs=100, is_training=True, current_stage=1, use_dense_trajectory=False, num_dense_points=100, prediction_type='x0'):
    ...
    # 生成初始轨迹控制点作为起点（而不是纯噪声）
    _, initial_middle_cp = generate_initial_trajectory_control_points(
        start_pose, goal_pose, num_middle_points=24, device=device
    )
    initial_middle_cp_normalized = initial_middle_cp / 20.0
    initial_middle_cp_normalized = torch.clamp(initial_middle_cp_normalized, -1.0, 1.0)
    noise_cp = initial_middle_cp_normalized
