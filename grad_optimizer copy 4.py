...
def cost_on_dense_trajectory(self, trajectory):
        """
        对已生成的密集轨迹直接计算 Cost。
        trajectory: [N, 3] (x, y, yaw)
        """
        ...
        # 2. 计算导数 (使用有限差分近似速度、加速度)
        # 假设时间步长 dt=1.0 (如果你的轨迹有对应总时间，请除以真实的 dt)
        dx = x[1:] - x[:-1]
        dy = y[1:] - y[:-1]
        ...
        grid_coords = self.world_to_grid_normalized(trajectory) # [1, N, 1, 2]
        ...
        # 4. 平滑性代价 (Smoothness Cost)
        # 使用二阶差分平方和
        smoothness_cost = torch.mean(ddx**2 + ddy**2 + ddyaw**2)

        # 5. 曲率代价 (Curvature Cost)
        eps = 1e-6
        # 速度近似 (对齐长度)
        v_sq = dx[1:]**2 + dy[1:]**2 + eps
        speed = torch.sqrt(v_sq)
        # 几何曲率公式: |x'y'' - y'x''| / (x'^2 + y'^2)^(3/2)
        geom_curvature = torch.abs(dx[1:] * ddy - dy[1:] * ddx) / (v_sq * speed + eps)
        ...
        # 6. 端点代价 (Endpoint Yaw Cost)
        # 惩罚轨迹末端与目标 yaw 的偏差
        yawn = self.end_pose[2]
        yaw_diff_end = torch.atan2(torch.sin(yaw[-1] - yawn), torch.cos(yaw[-1] - yawn))
        yaw_endpoint_cost = yaw_diff_end**2
...
class TrajectoryOptimizerSE2:
    ...
    def cost_on_poses(self, ctrl_poses):
        ...
        # i) 基于控制点间直线距离的均匀性损失
        # 计算包括端点在内的所有段的均匀性
        # 虽然起点和终点是detached的，但它们的位置仍参与前向计算
        # 梯度会推动预测点调整以让所有段（包括端点段）距离均匀
        N = ctrl_poses.shape[0]  # 控制点数量（包括起点和终点，通常是22）
                
        if N >= 2:
            # 1. 计算所有相邻控制点之间的直线距离（包括端点段）
            dx = ctrl_poses[1:, 0] - ctrl_poses[:-1, 0]  # (N-1,) = 21段
            dy = ctrl_poses[1:, 1] - ctrl_poses[:-1, 1]  # (N-1,)
            segment_distances = torch.sqrt(dx**2 + dy**2 + eps)  # 所有21段的距离
            ...
            if total_length > 1e-10:
                # 3. 计算理想的均匀段长度
                ideal_segment_length = total_length / (N - 1)  # 21段的平均长度
        ...
        # 首末段距离约束：防止第一个预测点和最后一个预测点距离起终点过远
        endpoint_distance_cost = torch.tensor(0.0, device=device)
        if N >= 3:  # 至少需要起点、一个中间点、终点
            # 第一段距离：start_pose 到 ctrl[1]（第一个预测点）
            first_seg_dist = torch.sqrt((ctrl[1, 0] - self.start_pose[0])**2 + 
                                       (ctrl[1, 1] - self.start_pose[1])**2)
            # 最后一段距离：ctrl[-2]（最后一个预测点）到 end_pose
            last_seg_dist = torch.sqrt((ctrl[-2, 0] - self.end_pose[0])**2 + 
                                      (ctrl[-2, 1] - self.end_pose[1])**2)
            
            # 计算中间段的平均距离作为参考
            if N >= 4:  # 至少有2个中间段
                middle_segments = segment_distances[1:-1]  # 排除首末段
                if len(middle_segments) > 0:
                    avg_middle_dist = torch.mean(middle_segments)
                    # 惩罚首末段距离超过中间段平均距离的情况
                    first_excess = torch.relu(first_seg_dist - avg_middle_dist * 1.5)  # 允许1.5倍
                    last_excess = torch.relu(last_seg_dist - avg_middle_dist * 1.5)
                    endpoint_distance_cost = first_excess**2 + last_excess**2
            
            # 备选方案：直接限制首末段的绝对距离
            # max_endpoint_dist = 2.0  # 最大允许的首末段距离
            # first_excess = torch.relu(first_seg_dist - max_endpoint_dist)
            # last_excess = torch.relu(last_seg_dist - max_endpoint_dist)
            # endpoint_distance_cost = first_excess**2 + last_excess**2
        ...
        total_cost = (
            ...
            weights['uniformity'] * uniformity_cost +
            weights['out_of_bounds'] * out_of_bounds_cost +
            weights['endpoint_distance'] * endpoint_distance_cost
        )
